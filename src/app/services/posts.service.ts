...
export class PostsService {
  ...
  // Add post in DB
  addPost(blogId: number, title: string, content: string, blog: Blog): Observable<Post> {
    
    // The api only allows the id 0 because it seems to take care of itself so this one is for the POST request
    const newPost: Post = {id: 0, title: title, content: content, created: new Date(), modified: new Date(), blogId: blogId, blog: blog, comments: []};
